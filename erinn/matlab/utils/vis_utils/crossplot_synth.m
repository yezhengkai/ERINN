function fig = crossplot_synth(synth_V, pred_V)
...
% 1. R squared
Ybar = mean(synth_V);
SS_tot = sum((synth_V - Ybar) .^ 2);
% SS_reg = sum((pred_V - Ybar) .^ 2);
SS_res = sum((synth_V - pred_V) .^ 2);
R2 = 1 - SS_res/SS_tot;
% 2. Root mean squared error
RMSE = sqrt(mean((synth_V - pred_V) .^ 2));
% 3. Root mean squared relative error
RMSRE = sqrt(mean(((synth_V - pred_V) ./ synth_V) .^ 2));
% 4. correlation coefficient
C = corrcoef(synth_V, pred_V);

%% get synth_V maximum and minimum
x_min = nanmin(synth_V); x_max = nanmax(synth_V);
